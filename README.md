...
[![Deploy na Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2Fnomicsadmin%2Fhub-agentes-ia&project-name=hub-agentes-ia&repository-name=hub-agentes-ia&env=NEXT_PUBLIC_SUPABASE_URL%2CNEXT_PUBLIC_SUPABASE_ANON_KEY%2CSUPABASE_SERVICE_ROLE_KEY%2COPENAI_API_KEY%2COPENAI_MODEL%2CNEXT_PUBLIC_SITE_URL%2CCRON_SECRET&envDescription=Chaves+do+Supabase+e+da+OpenAI.+Crie+o+banco+antes+%28docs%2F03-supabase.md%29.&envLink=https%3A%2F%2Fgithub.com%2Fnomicsadmin%2Fhub-agentes-ia%2Fblob%2Fmain%2Fdocs%2F09-deploy-vercel.md)

<sub>Open-source AI agent hub template · Next.js + Supabase + OpenAI · RAG chatbot with auth, payments and admin dashboard · <a href="#-english">English</a></sub>
...
<details>
<summary><b>Your own AI agent, trained on your method.</b> Click to read the summary.</summary>

<br>

**Hub de Agentes de IA** is a free, open-source template (MIT) for launching a private **AI agent hub** for your students, members or clients, in an afternoon.

- **Chat with streaming** answers, **audio messages** (transcribed), **image and PDF attachments**, and **PDFs generated by the agent**.
- **Agents trained on your material** (PDF, DOCX, MD): the whole knowledge base goes into the prompt up to ~80k tokens, with automatic **RAG** (pgvector) above that.
- **Auth by invitation** (Supabase), **payment webhooks** with a pluggable adapter (Hubla included; add Stripe, Hotmart, Kiwify...), access revoked on refund.
- **Admin dashboard**: active and inactive users (7/15/30 days), usage, **topics & difficulties** of your audience, knowledge **gaps**, agent prompt versions and **corrections without code**.
- **Mobile first**, installable **PWA**, light/dark themes, swappable **design system** (tokens in one file).
- **Setup assistant** (`/setup` and `npm run setup`), secret-free diagnostics, leak scanner, and a guided script so an AI (Claude Code, Cursor, Codex) builds the hub with you: *"Read COMECE-AQUI.md and build my agent hub."*
...
Docs are in Brazilian Portuguese; contributions with translations are welcome.
